...
public abstract class AbstractCEContainer<T extends Configuration> implements DeployableContainer<T>, DockerFileTemplateHandler {
    ...
    protected RunInPodContainer create(Archive<?> runInPodArchive) {
        return new RunInPodContainer(this, runInPodArchive);
    }
    ...
    public void setup(T configuration) {
        ...
        if (!isSPI() && RunInPodUtils.hasRunInPod(tc.get().getJavaClass())) {
            Archive<?> runAsPodArchive = RunInPodUtils.getRunAsPodArchive(tc.get().getJavaClass());
            runInPodContainer = create(runAsPodArchive);
        }
    }
